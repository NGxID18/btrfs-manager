...
const App = {
    ...
    async fetchDynamicMetrics() {
        for (let v of this.vols) {
            if (!v.mountPoint) { v.raid = "Mount required"; v.usable = "Locked"; this.updateUI(v); continue; }
            try {
    ...
    renderMaster() {
        ...
        $("disk-container").innerHTML = this.vols.length ? this.vols.map(v => `
            <div class="btrfs-card hoverable animated-view h-100-col">
                ...
                <p class="mb-15"><b>Mount Status:</b> ${v.mountPoint ? `<span class="text-success">${v.mountPoint}</span>` : `<span class="text-warning">Not Mounted</span> <span class="text-muted" style="font-size: 13px;">(Mount via Storage tab)</span>`}</p>
