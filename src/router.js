    ...
    try {
      switch (action) {
          ...
          customPrompt(
            ...
            (sz) => {
              if (sz)
                task(
                  `Resizing to ${sz}...`,
                  ["btrfs", "filesystem", "resize", sz, mnt],
                  "Resized!",
                  true,
                );
            },
        ...
        case "scrub":
          customConfirm(
            "Start Scrub",
            `Start data scrubbing on ${mnt}?`,
            "Start",
            () =>
              task(
                "Scrubbing...",
                ["btrfs", "scrub", "start", mnt],
                "Started. Click 'Check Scrub' for progress.",
              ),
          );
          break;
        case "scrub-status":
          task(
            "Fetching scrub status...",
        ...
        case "balance":
          customConfirm(
            "Start Balance",
            `Rebalance blocks (50% usage) on ${mnt}?`,
            "Start",
            () =>
              task(
                "Balancing...",
        ...
        case "manage-subvol":
          ...
          $("modal-subvol-path").innerText =
            "/" + tgt.getAttribute("data-path");
          $("modal-subvol-id").innerText =
            "(ID: " + tgt.getAttribute("data-subid") + ")";
          ...
          document
            ...
            .forEach((b) => {
              ...
              b.setAttribute("data-path", tgt.getAttribute("data-path"));
              b.setAttribute("data-subid", tgt.getAttribute("data-subid"));
...
if [ -n "$CFG_NAME" ] && [ "$CFG_NAME" != "Config" ]; then
    ...
    echo "No active Snapper config found for $TARGET, nothing to disable.";
fi
                                `;
...
printf "Snapper successfully configured!\\nConfig Name: %s\\nFrequency: %s\\nRetention Limit: %s\\n" "$CFG_NAME" "$FREQ" "$LIMIT"
          ...
          // FIX: cut -d'[' -f1 diterapkan di semua script router di bawah ini
          else if (op === "del") {
            customConfirm("Delete", `Delete "${p}"?`, "Delete", () => {
              const script = `MNT="$1"; SUB_PATH="$2"; DEV=$(findmnt -n -o SOURCE -T "$MNT" | head -n 1 | cut -d'[' -f1); if [ -z "$DEV" ]; then DEV=$(df "$MNT" | awk 'NR==2 {print $1}'); fi; TMP=$(mktemp -d); mount -t btrfs -o subvolid=5 "$DEV" "$TMP" || exit 1; btrfs subvolume delete "$TMP/$SUB_PATH"; RES=$?; umount "$TMP"; rmdir "$TMP"; exit $RES`;
            ...
            customPrompt(
              ...
              (n) => {
                if (n) {
                  const script = `MNT="$1"; SRC="$2"; DEST="$3"; DEV=$(findmnt -n -o SOURCE -T "$MNT" | head -n 1 | cut -d'[' -f1); if [ -z "$DEV" ]; then DEV=$(df "$MNT" | awk 'NR==2 {print $1}'); fi; TMP=$(mktemp -d); mount -t btrfs -o subvolid=5 "$DEV" "$TMP" || exit 1; if [ -z "$SRC" ]; then btrfs subvolume snapshot "$MNT" "$TMP/$DEST"; else btrfs subvolume snapshot "$TMP/$SRC" "$TMP/$DEST"; fi; RES=$?; umount "$TMP"; rmdir "$TMP"; exit $RES`;
            ...
            customPrompt(
              ...
              (n) => {
                if (n) {
                  const script = `MNT="$1"; SRC="$2"; DEST="$3"; DEV=$(findmnt -n -o SOURCE -T "$MNT" | head -n 1 | cut -d'[' -f1); if [ -z "$DEV" ]; then DEV=$(df "$MNT" | awk 'NR==2 {print $1}'); fi; TMP=$(mktemp -d); mount -t btrfs -o subvolid=5 "$DEV" "$TMP" || exit 1; btrfs subvolume snapshot "$TMP/$SRC" "$TMP/$DEST"; RES=$?; umount "$TMP"; rmdir "$TMP"; exit $RES`;
                  cmd(["sh", "-c", script, "--", mnt, p, n])
                    .then(() => App.fetchSubvols(mnt, i))
                    .catch((e) => customAlert("Failed", e.message));
                }
            ...
            customPrompt(
              ...
              (l) => {
                if (l) {
                  const script = `MNT="$1"; SUB_PATH="$2"; LIMIT="$3"; DEV=$(findmnt -n -o SOURCE -T "$MNT" | head -n 1 | cut -d'[' -f1); if [ -z "$DEV" ]; then DEV=$(df "$MNT" | awk 'NR==2 {print $1}'); fi; TMP=$(mktemp -d); mount -t btrfs -o subvolid=5 "$DEV" "$TMP" || exit 1; btrfs quota enable "$MNT"; btrfs qgroup limit "$LIMIT" "$TMP/$SUB_PATH"; RES=$?; umount "$TMP"; rmdir "$TMP"; exit $RES`;
            ...
            customConfirm(
              ...
              actionWord,
              () => {
                const script = `MNT="$1"; SUB_PATH="$2"; FLAG="$3"; DEV=$(findmnt -n -o SOURCE -T "$MNT" | head -n 1 | cut -d'[' -f1); if [ -z "$DEV" ]; then DEV=$(df "$MNT" | awk 'NR==2 {print $1}'); fi; TMP=$(mktemp -d); if mount -t btrfs -o subvolid=5 "$DEV" "$TMP"; then chattr "$FLAG" "$TMP/$SUB_PATH"; RES=$?; umount "$TMP"; rmdir "$TMP"; [ $RES -eq 0 ] && echo "Success" || exit 1; else rmdir "$TMP"; exit 1; fi`;
